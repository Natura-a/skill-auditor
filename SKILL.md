...
name: skill-auditor
description: |
  Agent Skill 工程化审计员。用于审查指定 Skill 的完整性与可用性，严格依据"契约式接口"原则。
  当用户明确下达"审查技能"、"审计xxx"、"检查所有技能"等指令时调用。
  支持定向审查和全量审查。具备增量跳过机制：若目标 Skill 的接口契约（Schema/返回格式/示例参数）未发生重大变更，且未使用强制参数，则直接返回历史评分，跳过深度 Token 消耗。
---
